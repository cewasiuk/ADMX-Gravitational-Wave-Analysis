...
for file_name in os.listdir(folder_path):
    ...
    if os.path.isfile(file_path):
        if df["Filename_Tag"].str.contains(match_name).any() == True:
            ...
            spectrum_start_freq = df.loc[df["Filename_Tag"].str.contains(match_name), "Start_Frequency"].iloc[0]
            spectrum_stop_freq = df.loc[df["Filename_Tag"].str.contains(match_name), "Stop_Frequency"].iloc[0]
            coupling_factor = 0.1
            ...
            f0 =  df.loc[df["Filename_Tag"].str.contains(match_name), "Cavity_Resonant_Frequency"].iloc[0] 
            center_freq = (spectrum_start_freq + spectrum_stop_freq)/2
    ...
    with h5py.File(file_path, "r") as file:
        ...
        fstart = spectrum_start_freq
        fstop = spectrum_stop_freq
    ...
    pow_freq= ADMXDataSeries(yvalues = yvalues, xstart = fstart, xstop = fstop, xunits = 'Hz',yunits = 'Power')
    spectrum_raw=PowerSpectrum( power = pow_freq.yvalues,start_freq = fstart, stop_freq = fstop) 
...
# Will need to likely rebin data to specified bin width 
    ...
    save_dataseries_to_hdf5(output_file,timestamp_id+"/filter_spectrum",filter_spectrum)
    ...
    output_file.attrs["fstart"] = fstart
    output_file.attrs["fstop"] = fstop
    output_file.attrs["std_dev"] = stdev
